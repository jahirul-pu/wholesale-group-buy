...
const campaignSchema = z.object({
  ...
  status: z.enum(['PENDING', 'ACTIVE', 'SUCCESS', 'FAILED']).default('PENDING'),
...
export default function EditCampaignPage({ params }: EditCampaignPageProps) {
  ...
  // Pre-fill form when campaign is fetched
  useEffect(() => {
    if (campaign) {
      setValue('title', campaign.title);
      setValue('basePrice', Number(campaign.basePrice));
      setValue('targetVolume', campaign.targetVolume);
      setValue('status', campaign.status);

      // format ISO datetime to datetime-local format (YYYY-MM-DDTHH:MM)
      if (campaign.startTime) {
        const startIso = new Date(campaign.startTime).toISOString().slice(0, 16);
        setValue('startTime', startIso);
      }
      if (campaign.endTime) {
        const endIso = new Date(campaign.endTime).toISOString().slice(0, 16);
        setValue('endTime', endIso);
      }

      setValue('shortDescription', campaign.shortDescription || '');
      setValue('richContent', campaign.richContent || '');

      if (campaign.images && Array.isArray(campaign.images)) {
        setValue('imagesList', campaign.images.map((url: string) => ({ url })));
      }

      if (campaign.specifications) {
        const specs = Object.entries(campaign.specifications).map(([key, value]) => ({
          key,
          value: String(value),
        }));
        setValue('specificationsList', specs);
      }

      if (campaign.tiers && Array.isArray(campaign.tiers)) {
        setValue('tiers', campaign.tiers.map((t: any) => ({
  ...
  const onSubmit = async (data: any) => {
    ...
    const payload = {
      ...
      status: data.startImmediately ? 'ACTIVE' : data.status,
    };

    try {
      const res = await fetch(`http://127.0.0.1:3000/api/admin/campaigns/${id}`, {
        method: 'PUT',
        headers: {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex justify-center items-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-slate-400">Loading campaign details...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto w-full bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        <div className="p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-white">Edit Campaign</h1>
              <p className="text-xs text-slate-400">Modify product spec sheets, carousel galleries, price tiers, or campaign status</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select
                  id="status"
                  {...register('status')}
                  className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-xl px-4 py-3 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/15"
                >
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden">
                <div className="flex justify-between items-center bg-slate-900 border-b border-slate-800 px-4 py-2">
                  ...
                  {editorTab === 'edit' && (
                    <div className="flex items-center gap-1.5 text-xs text-slate-505">
                      <button type="button" onClick={() => insertMarkdown('bold')} className="p-1.5 hover:bg-slate-800 hover:text-slate-205 rounded font-bold" title="Bold">B</button>
                      <button type="button" onClick={() => insertMarkdown('italic')} className="p-1.5 hover:bg-slate-800 hover:text-slate-205 rounded italic" title="Italic">I</button>
                      <button type="button" onClick={() => insertMarkdown('heading')} className="p-1.5 hover:bg-slate-800 hover:text-slate-205 rounded font-semibold text-[10px]" title="Heading">H2</button>
                      <button type="button" onClick={() => insertMarkdown('list')} className="p-1.5 hover:bg-slate-800 hover:text-slate-205 rounded font-mono" title="Bullet List">• List</button>
                    </div>
                  )}
                </div>
                ...
                {editorTab === 'edit' ? (
                  ...
                  <div
                    className="p-4 min-h-[160px] bg-slate-950 text-slate-202 text-xs overflow-y-auto space-y-1.5 select-none"
                    dangerouslySetInnerHTML={{ __html: parseMarkdown(richContentText) || '<p class="text-slate-650 italic text-center py-6">No markdown content yet.</p>' }}
                  />
                )}
              </div>
            </div>
                      ...
                      <div key={field.id} className="flex gap-4 items-start bg-slate-950/40 p-4 rounded-xl border border-slate-800/80">
                        ...
                        <div className="flex items-center gap-1 mt-4">
                          <button
                            ...
                            className="text-slate-500 hover:text-slate-200 transition p-1.5 disabled:opacity-30"
                            title="Move Up"
                          >
                          ...
                          <button
                            ...
                            onClick={() => swapImages(index, index + 1)}
                            className="text-slate-505 hover:text-slate-202 transition p-1.5 disabled:opacity-30"
                            title="Move Down"
                          >
            ...
            {/* Section 5: Price Tiers */}
            <div className="space-y-6">
              <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                <h2 className="text-sm font-semibold text-emerald-400 flex items-center gap-2">
                  <DollarSign className="h-4 w-4" />
                  5. Pricing Drop Tiers
                </h2>
                <button
                  type="button"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            ...
                            className={`w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none ${
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-505 text-xs">$</span>
                            <input
                              type="number"
                              step="0.01"
                              {...register(`tiers.${index}.unlockedPrice` as const)}
                              className={`w-full bg-slate-950 border rounded-lg pl-6 pr-3 py-2 text-xs text-slate-100 focus:outline-none ${
